...
public class MultidimensionalArrayBynewExample {
	...
	public static void main(String[] args) {
		// 각 반의 학생 수가 3명으로 동일할 경우 점수 저장을 위한 2차월 배열 생성
		int[][] mathScroes = new int[2][3];
		// 배열 항목 초기값 출력

		for (int i = 0; i < mathScroes.length; i++) { // 반의 수만큼 반복
			for (int k = 0; k < mathScroes[i].length; k++) { // 해당 반의 학생 수만큼 반복
		...
		for (int i = 0; i < mathScroes.length; i++) { // 반의 수만큼 반복
			totalStudent = mathScroes[i].length; // 반의 학생 수 합산
			for (int k = 0; k < mathScroes[i].length; k++) { // 해당 반의 학생 수만큼 반복
				totalMathSum += mathScroes[i][k]; // 학생 점수 합산
			}
		}
		...
		for (int i = 0; i < englishScores.length; i++) { // 반의 수만큼 반복
			totalStudent += englishScores[i].length; // 반의 학생 수만큼 반복
